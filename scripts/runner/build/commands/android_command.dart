...
///
/// `android_command sample`
/// A [Command] to exemplify a sub command
/// {@endtemplate}
const androidConfigPath = 'android/versioning.properties';
...
class AndroidCommand extends Command<int> {
  ...
  @override
  String get description => '对Android包进行预构建';
  ...
  Future<int> run() async {
    ...
    if (await GitDir.isGitDir(path.current)) {
      ...
      final baseConfigFile = File(androidConfigPath);
      if (baseConfigFile.existsSync()) {
        _logger.warn('File $androidConfigPath is deleted.');
        baseConfigFile.deleteSync();
        // final content = baseConfigFile.readAsStringSync();
        // _logger.info(content);
      }
      _logger.warn('File $androidConfigPath is created.');
      baseConfigFile
        ..createSync()
        ..writeAsStringSync('app.env=$env');
      if (gitCommit != null && gitCommit.isNotEmpty) {
        baseConfigFile.writeAsStringSync(
          '\napp.commit=$gitCommit',
